---
name: documents/docs.cloud.google.com/gemini/docs/codeassist/use-agentic-chat-pair-programmer
uri: https://docs.cloud.google.com/gemini/docs/codeassist/use-agentic-chat-pair-programmer
title: Use the Gemini Code Assist agent mode
description: Pair program with Gemini Code Assist agent mode chat.
...
Some features of [standard Gemini Code Assist chat](https://docs.cloud.google.com/gemini/docs/codeassist/chat-overview) might not be available in agent mode or might work differently than they do in standard chat.

Recitation is not available in agent mode. While in agent mode, Gemini doesn't [cite sources](https://docs.cloud.google.com/gemini/docs/discover/works#how-when-gemini-cites-sources) and you can't [disable code suggestions that match cited sources](https://docs.cloud.google.com/gemini/docs/codeassist/write-code-gemini#disable_code_suggestions_that_match_cited_sources) .

## Before you begin

1.  [Set up Gemini Code Assist](https://docs.cloud.google.com/gemini/docs/codeassist/set-up-gemini) .

## Use agent mode

In agent mode, you can ask Gemini to complete high-level goals and complex tasks.

To get the most out of agent mode, follow [prompting best practices](https://docs.cloud.google.com/gemini/docs/discover/write-prompts) and provide as much detail as possible.
...
To stop the agent, click stop **Stop** .

To use the standard Gemini Code Assist chat, click add **New chat** to create a new chat.

### IntelliJ

1.  Click spark **Gemini** in the tool window bar. Sign in if prompted to do so.
...
2.  Write any rules, style guide information, or context that you want the agent to use in Markdown and save the file. For more information, see the [example context file on GitHub](https://geminicli.com/docs/cli/gemini-md/) .
...
Gemini Code Assist includes different daily [quotas](https://docs.cloud.google.com/gemini/docs/quotas#quotas-for-agent-mode-gemini-cli) for agentic features, depending on the tier you're in.

If you've exhausted your daily quota for Gemini Code Assist agent mode, you can continue to use the service by providing an API key. You can use either a [Gemini API key](https://ai.google.dev/gemini-api/docs/api-key) or a [Vertex AI API key](https://docs.cloud.google.com/gemini-enterprise-agent-platform/models/start/api-keys) .
