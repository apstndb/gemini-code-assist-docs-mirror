---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/gemini/code-tools-settings/setting-bindings/list
uri: https://docs.cloud.google.com/sdk/gcloud/reference/gemini/code-tools-settings/setting-bindings/list
title: gcloud gemini code-tools-settings setting-bindings list
...
`gcloud gemini code-tools-settings setting-bindings list` ( `  --code-tools-setting  ` = `  CODE_TOOLS_SETTING  ` : `  --location  ` = `  LOCATION  ` ) \[ `  --filter  ` = `  EXPRESSION  ` \] \[ `  --limit  ` = `  LIMIT  ` \] \[ `  --page-size  ` = `  PAGE_SIZE  ` \] \[ `  --sort-by  ` =\[ `  FIELD  ` , …\]\] \[ `  --uri  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
CodeToolsSetting resource - Parent value for ListSettingBindingsRequest. The arguments in this group can be used to specify the attributes of this resource. (NOTE) Some attributes are not given arguments in this group but can be set in other ways.
