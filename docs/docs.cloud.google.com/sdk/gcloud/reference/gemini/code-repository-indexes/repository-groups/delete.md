---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/gemini/code-repository-indexes/repository-groups/delete
uri: https://docs.cloud.google.com/sdk/gcloud/reference/gemini/code-repository-indexes/repository-groups/delete
title: gcloud gemini code-repository-indexes repository-groups delete
...
`gcloud gemini code-repository-indexes repository-groups delete` ( `  REPOSITORY_GROUP  ` : `  --code-repository-index  ` = `  CODE_REPOSITORY_INDEX  ` `  --location  ` = `  LOCATION  ` ) \[ `  --async  ` \] \[ `  --request-id  ` = `  REQUEST_ID  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
To delete a repository group `my-repository-group` from a code repository index instance `my-instance` in project `my-project` and location `us-central1` with a fully qualified name, run:
