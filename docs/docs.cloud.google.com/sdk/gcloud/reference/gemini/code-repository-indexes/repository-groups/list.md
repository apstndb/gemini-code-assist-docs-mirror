---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/gemini/code-repository-indexes/repository-groups/list
uri: https://docs.cloud.google.com/sdk/gcloud/reference/gemini/code-repository-indexes/repository-groups/list
title: gcloud gemini code-repository-indexes repository-groups list
...
`gcloud gemini code-repository-indexes repository-groups list` ( `  --code-repository-index  ` = `  CODE_REPOSITORY_INDEX  ` : `  --location  ` = `  LOCATION  ` ) \[ `  --filter  ` = `  EXPRESSION  ` \] \[ `  --limit  ` = `  LIMIT  ` \] \[ `  --page-size  ` = `  PAGE_SIZE  ` \] \[ `  --sort-by  ` =\[ `  FIELD  ` , …\]\] \[ `  --uri  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
To list all repository groups of a code repository index instance `my-instance` in project `my-project` and location `us-central1` , run:
...
CodeRepositoryIndex resource - Parent value for ListRepositoryGroupsRequest The arguments in this group can be used to specify the attributes of this resource. (NOTE) Some attributes are not given arguments in this group but can be set in other ways.
