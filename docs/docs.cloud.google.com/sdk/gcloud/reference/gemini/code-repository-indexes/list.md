---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/gemini/code-repository-indexes/list
uri: https://docs.cloud.google.com/sdk/gcloud/reference/gemini/code-repository-indexes/list
title: gcloud gemini code-repository-indexes list
...
`gcloud gemini code-repository-indexes list` `  --location  ` = `  LOCATION  ` \[ `  --filter  ` = `  EXPRESSION  ` \] \[ `  --limit  ` = `  LIMIT  ` \] \[ `  --page-size  ` = `  PAGE_SIZE  ` \] \[ `  --sort-by  ` =\[ `  FIELD  ` , …\]\] \[ `  --uri  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
To list all code repository index instances in project `my-project` and location `us-central` , run:
    ...
    gcloud gemini code-repository-indexes list --project=my-project --location=us-central1

REQUIRED FLAGS

Location resource - Parent value for ListCodeRepositoryIndexesRequest This represents a Cloud resource. (NOTE) Some attributes are not given arguments in this group but can be set in other ways.
