---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/gemini/data-sharing-with-google-settings/setting-bindings/list
uri: https://docs.cloud.google.com/sdk/gcloud/reference/gemini/data-sharing-with-google-settings/setting-bindings/list
title: gcloud gemini data-sharing-with-google-settings setting-bindings list
...
`gcloud gemini data-sharing-with-google-settings setting-bindings list` ( `  --data-sharing-with-google-setting  ` = `  DATA_SHARING_WITH_GOOGLE_SETTING  ` : `  --location  ` = `  LOCATION  ` ) \[ `  --filter  ` = `  EXPRESSION  ` \] \[ `  --limit  ` = `  LIMIT  ` \] \[ `  --page-size  ` = `  PAGE_SIZE  ` \] \[ `  --sort-by  ` =\[ `  FIELD  ` , …\]\] \[ `  --uri  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
DataSharingWithGoogleSetting resource - Parent value for ListSettingBindingsRequest. The arguments in this group can be used to specify the attributes of this resource. (NOTE) Some attributes are not given arguments in this group but can be set in other ways.
