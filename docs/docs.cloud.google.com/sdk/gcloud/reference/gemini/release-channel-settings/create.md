...
`gcloud gemini release-channel-settings create` ( `  RELEASE_CHANNEL_SETTING  ` : `  --location  ` = `  LOCATION  ` ) \[ `  --labels  ` =\[ `  LABELS  ` , …\]\] \[ `  --release-channel  ` = `  RELEASE_CHANNEL  ` \] \[ `  --request-id  ` = `  REQUEST_ID  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
