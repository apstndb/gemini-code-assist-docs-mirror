---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/gemini/logging-settings/list
uri: https://docs.cloud.google.com/sdk/gcloud/reference/gemini/logging-settings/list
title: gcloud gemini logging-settings list
...
`gcloud gemini logging-settings list` `  --location  ` = `  LOCATION  ` \[ `  --filter  ` = `  EXPRESSION  ` \] \[ `  --limit  ` = `  LIMIT  ` \] \[ `  --page-size  ` = `  PAGE_SIZE  ` \] \[ `  --sort-by  ` =\[ `  FIELD  ` , …\]\] \[ `  --uri  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
Location resource - Parent value for ListLoggingSettingsRequest. This represents a Cloud resource. (NOTE) Some attributes are not given arguments in this group but can be set in other ways.
