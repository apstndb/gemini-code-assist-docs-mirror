---
name: documents/docs.cloud.google.com/sdk/gcloud/reference/gemini/logging-settings/update
uri: https://docs.cloud.google.com/sdk/gcloud/reference/gemini/logging-settings/update
title: gcloud gemini logging-settings update
...
`gcloud gemini logging-settings update` ( `  LOGGING_SETTING  ` : `  --location  ` = `  LOCATION  ` ) \[ `  --[no-]log-metadata  ` \] \[ `  --[no-]log-prompts-and-responses  ` \] \[ `  --request-id  ` = `  REQUEST_ID  ` \] \[ `  --labels  ` =\[ `  LABELS  ` , …\] | `  --update-labels  ` =\[ `  UPDATE_LABELS  ` , …\] `  --clear-labels  ` | `  --remove-labels  ` = `  REMOVE_LABELS  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
