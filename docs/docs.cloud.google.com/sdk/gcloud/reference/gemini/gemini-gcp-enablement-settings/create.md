...
`gcloud gemini gemini-gcp-enablement-settings create` ( `  GEMINI_GCP_ENABLEMENT_SETTING  ` : `  --location  ` = `  LOCATION  ` ) \[ `  --custom-instructions  ` = `  CUSTOM_INSTRUCTIONS  ` \] \[ `  --disable-web-grounding  ` \] \[ `  --enable-customer-data-sharing  ` \] \[ `  --gemini-enterprise-project  ` = `  GEMINI_ENTERPRISE_PROJECT  ` \] \[ `  --labels  ` =\[ `  LABELS  ` , …\]\] \[ `  --mutations-enabled  ` \] \[ `  --proactive-agents-enabled  ` \] \[ `  --release-channel  ` = `  RELEASE_CHANNEL  ` \] \[ `  --request-id  ` = `  REQUEST_ID  ` \] \[ `  --web-grounding-type  ` = `  WEB_GROUNDING_TYPE  ` \] \[ `  GCLOUD_WIDE_FLAG …  ` \]
...
FLAGS
  ...
  - `--disable-web-grounding`  
    Whether web grounding should be disabled. DEPRECATED: Use web\_grounding\_type instead.
